...
// variables (prefixed with `CYBER_2_K8S`) and finally merges in flags from the flagSet.
...
//
// Can be set in the config file as `foo_bar="baz"`, in the environment as `CYBER_2_K8S_FOO_BAR=baz`,
// or via the command line flag `--foo-bar=baz`.
func Load(configFileName string, flagSet *pflag.FlagSet, into any) error {
	v := viper.New()
	v.SetConfigFile(configFileName)
	v.SetConfigType("toml") // Config is in toml format
	v.SetEnvPrefix("CYBER_2_K8S")
	v.AutomaticEnv()
...
func registerFlags(v *viper.Viper, prefix string, flagSet *pflag.FlagSet, options any) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if field.Type.Kind() == reflect.Struct {
			if cfgName != ",squash" {
				return fmt.Errorf("filed %q does not have required cfg tag: `,squash`", fieldName)
			}
...
// decodeFromCfgTag sets the Viper decoder to read the names from the `cfg` tag on
// each struct entry.
...
func LoadYAML(configFileName string, into any) error {
	...
	// not mapped to fields of the int structure
...
func loadAndParseYaml(configFileName string) ([]byte, error) {
	...
	modifiedBuffer := normalizeSubstitution(unparsedBuffer)
